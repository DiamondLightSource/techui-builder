...
def find_services_folders(component: Component):

    services_directory = component.P + "-services/services"  # Will be changed, probably made relative to actual directory i.e. "./services" or absolute paths.
    ...
    for file in files: 
        ...
        if match:
            if match.group(1) == component.P.lower():
                print(match.group(1))
                print(match.group(2))
...
def extract_valid_entities(iocs_yaml, component: Component):
    ...
    for ioc_yaml in iocs_yaml:
        with open(ioc_yaml, "r") as ioc:
            ...
            for entity in entities:
                if 'P' in entity.keys() and entity['P'] == component.P+":"+ component.R: # the suffix could be M, could be R
